...
class SynthesizeSpeechCommand:
    ...
    def __post_init__(self) -> None:
        if not self.model_profile_id.strip():
            raise VoiceTtsUsageError("model_profile_id must not be blank")
        if not self.text.strip():
            raise VoiceTtsUsageError("text must not be blank")
        if not self.prompt_text.strip():
            raise VoiceTtsUsageError("prompt_text must not be blank")
        if not str(self.ref_audio_path).strip():
            raise VoiceTtsUsageError("ref_audio_path must not be blank")
        if self.ref_start_sec is not None and self.ref_start_sec < 0.0:
            raise VoiceTtsUsageError("ref_start_sec must be >= 0.0")
        effective_start = self.ref_start_sec or 0.0
        if self.ref_end_sec is not None and self.ref_end_sec <= effective_start:
            raise VoiceTtsUsageError("ref_end_sec must be greater than ref_start_sec")
        if self.output_path is not None and self.output_path.suffix.lower() != ".wav":
            raise VoiceTtsUsageError("output_path must end with .wav")
    ...
    def resolve_output_path(self, output_root: Path) -> Path:
        ...
        if resolved_path.exists() and resolved_path.is_dir():
            raise VoiceTtsUsageError(f"output path points to a directory: {resolved_path}")
    ...
    def to_domain(self, output_path: Path) -> SynthesisRequest:
        try:
            ...
            raise VoiceTtsUsageError(str(exc)) from exc
