...
| `FEAT-CLI-01` | Local CLI Surface | `live` | `voice-tts version`, `voice-tts init`, `voice-tts doctor`, `voice-tts profiles`, `voice-tts prepare-ref`, `voice-tts synthesize`를 통해 first-run setup부터 로컬 WAV 합성까지 실행할 수 있다. |
...
| `FEAT-REF-01` | Reference-Audio Assist | `live` | `prepare-ref`가 source metadata를 보여주고 silence-based candidate ranking 또는 explicit trim export를 수행한다. |
...
| `FEAT-ARCH-01` | Clean Architecture Seams | `live` | `SpeechSynthesisEngine`, `ModelProfileRepository`, `ReferenceAudioPreparationService`, use case seam이 유지된다. |
...
| `FEAT-CLI-01` | `none` | 개발자 | 첫 실행 설정부터 ref prep와 합성까지 로컬 작업 흐름을 실행한다 | `uv run voice-tts init`, `uv run voice-tts doctor`, `uv run voice-tts prepare-ref`, `uv run voice-tts synthesize` | `live` | external checkout setup이 prerequisite다 |
...
| `FEAT-REF-01` | `FEAT-CLI-01` | 개발자 | source audio를 inspect하고 ranked ref clip을 준비한다 | `voice_tts.application.use_cases.PrepareReferenceAudioUseCase` | `live` | ASR 없이 trim/export만 수행한다 |
...
| --- | --- |
| 구현을 이어갈 개발자 | 문서와 CLI를 읽고 first-run setup부터 local synthesis path까지 바로 실행하거나 확장할 수 있다. |
...
| reference clip을 준비하려는 개발자 | `voice-tts prepare-ref`로 source audio metadata를 보고 ranked candidate를 선택하거나 explicit trim으로 WAV clip을 만든다. |
...
### `FLOW-CLI-03` First-Run Setup and Profile Discovery

- `Entry`: `uv run voice-tts init`, `uv run voice-tts profiles`
- `Path`: GPT-SoVITS root 입력 -> `.env`/manifest scaffold -> seeded profile 생성 -> profile metadata와 config path status 출력
- `Current Surface`: CLI, workspace scaffolding helper, model profile repository
- `Current Hold Boundary`: multi-profile authoring UI는 없고 single seeded profile bootstrap이 기본이다

### `FLOW-CLI-04` Reference-Audio Preparation

- `Entry`: `uv run voice-tts prepare-ref`
- `Path`: source audio inspect -> duration/sample-rate/channels/format 출력 -> silence-based candidate ranking 또는 explicit range 선택 -> optional WAV export
- `Current Surface`: CLI, application/use case, ffmpeg/ffprobe-backed reference audio adapter
- `Current Hold Boundary`: ASR, diarization, VAD는 하지 않으며 clip suggestion과 export까지만 수행한다

### `FLOW-CLI-05` Local Zero-shot Synthesis
