...
| `ARCH-INF-01` | Infrastructure | 외부 시스템 adapter, runtime integration | GPT-SoVITS, filesystem, manifest parsing, ffmpeg trim, config loader, logging, shallow import helper를 격리한다 | Open |
...
3. `src/voice_tts/infrastructure`는 settings, logging, repository adapter, engine adapter, external command helper, GPT-SoVITS import helper를 소유한다.
...
4. synthesis 성공/실패는 deterministic metadata 또는 stage-prefixed error로 surface에 올라와야 한다.
...
7. profile compatibility는 typed contract로 유지하며 `required_checkout_files`와 `supported_devices`가 최소 canonical set이다.

## 7. Current Repository Drift

| Drift | Why It Matters |
| --- | --- |
| reference-audio assist가 아직 없다 | current trim flow는 강하지만 사람 개입이 필요하다 |
