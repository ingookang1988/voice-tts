...
| Project Packaging | `working` | `uv + pyproject` 기반 Python project가 존재하며 `numpy`, `soundfile`가 추가되었다 | `pyproject.toml`, `.python-version`, `README.md` |
...
| Model Profile Repository | `working` | JSON manifest에서 `id/version/tts_config_path` 기반 profile을 읽는다 | `src/voice_tts/infrastructure/repositories.py`, `config/model-profiles.example.json` |
| GPT-SoVITS Runtime Adapter | `working` | external checkout을 `sys.path`에 붙여 GPT-SoVITS v2 inference 클래스를 import한다 | `src/voice_tts/infrastructure/engines.py` |
...
| Doctor Diagnostics | `working` | Python version, device policy, workdir/temp/output, GPT root, ffmpeg, manifest profile count를 점검한다 | `src/voice_tts/bootstrap/doctor.py` |
...
1. 현재 repo는 local-first CLI synthesis 단계다.
2. `doctor`는 inference를 수행하지 않지만 Phase 2 synthesis prerequisites를 fail-fast로 검사한다.
...
5. real upstream execution은 local environment에 external GPT-SoVITS checkout과 compatible dependencies가 준비되어야 한다.
...
| infrastructure | `src/voice_tts/infrastructure/config.py`, `src/voice_tts/infrastructure/logging.py`, `src/voice_tts/infrastructure/repositories.py`, `src/voice_tts/infrastructure/engines.py`, `src/voice_tts/infrastructure/system.py` |
...
Phase 2에서 유지해야 하는 verification baseline:
...
| profile metadata contract가 얇다 | manifest는 최소 필드만 가진다 | compatibility UX와 richer selection은 다음 단계다 |
| auto speaker cleanup가 없다 | manual trim flags only | multi-speaker audio prep는 여전히 수동 작업이 필요하다 |
